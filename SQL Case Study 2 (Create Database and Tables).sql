...
From DIM_LOCATION T1 
		  INNER JOIN FACT_TRANSACTIONS P1 ON T1.IdLocation = P1.IdLocation
Where YEAR(P1.DATE) >= 2005

--Q2.
Select Distinct(C1.STATE) 
From DIM_MODEL T1 
		 INNER JOIN FACT_TRANSACTIONS P1 ON T1.IdModel = P1.IdModel
		 INNER JOIN DIM_LOCATION C1 ON P1.IdLocation = C1.IdLocation 
		 INNER JOIN DIM_MANUFACTURER D1 ON T1.IDManufacturer = D1.IDManufacturer
Where D1.Manufacturer_Name = 'SAMSUNG' AND C1.Country = 'US'


--Q3.
Select Count(*) As NO_OF_TRANSACTIONS
From 
   (Select Distinct T1.IDMODEL, P1.ZIPCODE, P1.State
	From FACT_TRANSACTIONS T1
				 LEFT JOIN DIM_LOCATION P1   ON T1.IdLocation = P1.IdLocation
	Group By T1.IDMODEL, P1.ZIPCODE, P1.State 
    )As TT	
...
--Q5:
Select T1.MODEL_NAME, Round(AVG(P1.TOTALPRICE),2) As Avg_Price
From DIM_MODEL T1 
	  LEFT JOIN FACT_TRANSACTIONS P1   ON   T1.IDModel = P1.IDModel
	  LEFT JOIN DIM_MANUFACTURER C1   ON   T1.IDManufacturer = C1.IDManufacturer 
Where C1.Manufacturer_Name IN  (
								Select TOP 5 (C1.Manufacturer_Name) 
								From DIM_MODEL T1 
										  LEFT JOIN FACT_TRANSACTIONS   P1  ON T1.IDModel = P1.IDModel
										  LEFT JOIN DIM_MANUFACTURER     C1   ON T1.IDManufacturer = C1.IDManufacturer 
								Group By C1.Manufacturer_Name
								Order By Sum(P1.Quantity) DESC
								)
Group By T1.Model_Name	
Order By AVG(P1.TOTALPRICE) DESC

--Q6.
Select T1.CUSTOMER_NAME, AVG(P1.TotalPrice) As Amt_Spent 
From DIM_CUSTOMER T1 
		 INNER JOIN FACT_TRANSACTIONS   P1  ON   T1.IdCustomer = P1.IdCustomer
Where YEAR(P1.DATE) = 2009
Group By T1.Customer_Name
Having AVG(P1.TOTALPRICE) > 500

--Q7:
Select * From
		(Select * From 
					(Select TOP 5 T1.Model_Name 
					 From DIM_MODEL T1 
							 LEFT JOIN FACT_TRANSACTIONS P1  ON T1.IdModel = P1.IdModel 
					 Where YEAR(DATE) = 2008
					 Group By T1.Model_Name
					 Order By Sum(P1.Quantity) DESC)X
UNION ALL
Select * From 
			(Select TOP 5 T1.Model_Name  
			From DIM_MODEL T1 
...
--Q8:
Select Distinct(YEAR(C1.DATE)) As YEAR, T1.MANUFACTURER_NAME
From DIM_MANUFACTURER T1 
		LEFT JOIN DIM_MODEL P1 ON T1.IdManufacturer = P1.IdManufacturer
		LEFT JOIN FACT_TRANSACTIONS C1 ON P1.IdModel = C1.IdModel
Where 
	(YEAR(C1.DATE) = 2009)
		AND C1.TotalPrice =  (Select Max(TotalPrice) 
...
From DIM_MANUFACTURER T1 
		INNER JOIN DIM_MODEL P1 ON T1.IdManufacturer = P1.IdManufacturer 
		INNER JOIN FACT_TRANSACTIONS C1 ON P1.IdModel = C1.IdModel
Where YEAR(C1.DATE) = 2010 AND YEAR(C1.DATE) != 2009

--Q10:
Select TOP 100 T1.CUSTOMER_NAME,
AVG(Case When YEAR(P1.DATE) = 2003 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2003,
AVG(Case When YEAR(P1.DATE) = 2003 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2003,
AVG(Case When YEAR(P1.DATE) = 2004 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2004,
AVG(Case When YEAR(P1.DATE) = 2004 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2004,
AVG(Case When YEAR(P1.DATE) = 2005 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2005,
AVG(Case When YEAR(P1.DATE) = 2005 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2005,
AVG(Case When YEAR(P1.DATE) = 2006 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2006,
AVG(Case When YEAR(P1.DATE) = 2016 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2006,
AVG(Case When YEAR(P1.DATE) = 2007 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2007,
AVG(Case When YEAR(P1.DATE) = 2007 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2007,
AVG(Case When YEAR(P1.DATE) = 2008 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2008,
AVG(Case When YEAR(P1.DATE) = 2008 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2008,
AVG(Case When YEAR(P1.DATE) = 2009 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2009,
AVG(Case When YEAR(P1.DATE) = 2009 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2009,
AVG(Case When YEAR(P1.DATE) = 2010 Then (P1.TOTALPRICE) END) As AVG_SPENT_IN_2010,
AVG(Case When YEAR(P1.DATE) = 2010 Then (P1.QUANTITY) END) As AVG_QUANTITY_IN_2010											
From DIM_CUSTOMER T1 
		LEFT JOIN FACT_TRANSACTIONS P1 ON T1.IdCustomer = P1.IdCustomer
Group By  T1.CUSTOMER_NAME
Order By Sum(P1.TOTALPRICE)
